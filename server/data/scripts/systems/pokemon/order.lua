...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if not summon then
        player:sendCancelMessage("Voc� n�o tem um Pok�mon ativo!")
        return false
    end
    
    summon:setPokeStopped(true) -- Faz o Pok�mon parar
...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if not hasSummons(player) and hasRideOrFlyStorage(player) then
        ...
        player:setFly(false)
        -- Refresh walk cache so water tiles are correctly blocked again after fly ends.
    ...
    if target:isItem() then
        if target:isItem() then
            ...
            if not ground or not destTile or destTile.house then
                return true
            end
			...
			local phrases = {
				"V� at� l� " .. summonName .. "!",
				summonName .. ", mova-se!",
				"R�pido " .. summonName .. ", ande!",
				"Em movimento, " .. summonName .. "!",
				summonName .. ", n�o fique parado!"
			}
    ...
    if distance < 3 then
        ...
        if monsterType then
            ...
            if flyOutfit > 0 then
                ...
                master:setFly(true)
                -- Refresh walk cache so auto-walk pathfinding can route through water tiles.
                master:refreshWalkCache()
