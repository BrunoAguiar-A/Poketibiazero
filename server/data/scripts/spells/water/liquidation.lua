...
local function doShotDownDamage(creature_id, toPos, target_id, xOffset, yOffset)
    ...
    local effectPosition = Position(target:getPosition().x + xOffset + 2, target:getPosition().y + yOffset, target:getPosition().z)
...
local function doShot(count, creature_id, target_id, xOffset, yOffset)
    if count > 0 then
        local creature = Creature(creature_id)
        if not creature then
            return
...
function spell.onCastSpell(creature, variant)
    ...
    -- Inicialize xOffset e yOffset com valores padrão
    local xOffset = 0
    local yOffset = 0

    doShot(SHOT_COUNT, creature_id, target_id, xOffset, yOffset)
    return true
end
