...
local maxLevel = PokemonLevel and PokemonLevel.CONFIG and PokemonLevel.CONFIG.maxLevel or cfg.max

	if boost >= maxLevel then
	...
	local chanceFail = math.random(1, 100)
        if chanceFail <= cfg.chance then
            if PokemonLevel and PokemonLevel.addLevel then
                PokemonLevel.addLevel(target, cfg.qnt)
            else
