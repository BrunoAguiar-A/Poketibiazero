function onUse(player, item, fromPosition, target, toPosition)
    ...
    local maxBoostValue = PokemonLevel and PokemonLevel.CONFIG and PokemonLevel.CONFIG.progression and PokemonLevel.CONFIG.progression.maxBoost or 50000
    local boost = PokemonLevel and PokemonLevel.getBoost and PokemonLevel.getBoost(target, false) or (target:getSpecialAttribute("pokeBoost") or 0)
    ...
    for _, stone in ipairs(cfg) do
        if item:getId() == stone.id then
            ...
            if chanceFail <= failChance then
                ...
                local newBoost = math.min(maxBoostValue, boost + stone.qnt)
                if PokemonLevel and PokemonLevel.setBoost then
                    newBoost = PokemonLevel.setBoost(target, newBoost)
                else
                    target:setSpecialAttribute("pokeBoost", newBoost)
                end
                player:getPosition():sendMagicEffect(896)
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Seu pokémon avançou do nível [+" .. tonumber(boost) .. "] de boost para o nível [+" .. tonumber(newBoost) .. "].")
                item:remove(1)
            end
