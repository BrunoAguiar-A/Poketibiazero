function onLogout(player)
	...
	for i = 1, #pokeballs do
	    ...
	    if PokemonLevel and PokemonLevel.ensureBall then
	    	PokemonLevel.ensureBall(pokeballs[i], name)
	    end
	    local level = PokemonLevel and PokemonLevel.getLevel and PokemonLevel.getLevel(pokeballs[i]) or (pokeballs[i]:getSpecialAttribute("pokeLevel") or 0)
	    pokemon.level = level
