		...
		if player then
			if #player:getSummons() < 1 then
				...
				if player:removeItem(item, 5) then
					...
					local pokeName = ball:getSpecialAttribute("pokeName") or summon:getName()
					local pokeLevel = PokemonLevel and PokemonLevel.getLevel and PokemonLevel.getLevel(ball) or (ball:getSpecialAttribute("pokeLevel") or 1)
					local pokeBoost = ball:getSpecialAttribute("pokeBoost") or 0
					local overheadName = PokemonLevel and PokemonLevel.formatOverheadName and PokemonLevel.formatOverheadName(pokeName, pokeLevel, pokeBoost, true)
						or (pokeBoost >= 1 and string.format("%s [%d +%d]", pokeName, pokeLevel, pokeBoost) or string.format("%s [%d]", pokeName, pokeLevel))
