...
function onStepIn(creature, item, position, fromPosition)
    ...
    -- Se o player está voando, pode cruzar água sem restrições de surf
    if creature:getStorageValue(storageFly) > 0 then
        return true
    end
...
function onStepOut(creature, item, position, fromPosition)
    ...
    -- If player is flying, don't trigger surf exit logic (avoids duplicate summon)
    if creature:getStorageValue(storageFly) > 0 then
        return true
    end
    ...
    -- Only exit surf if surf was actually active (storageSurf > 0).
    -- If the player flew over water and then exited fly near water, storageSurf is -1
    -- and we must NOT call doReleaseSummon (which would create a duplicate summon).
    if ((not isInArray(waterIds, tileId) and position == fromPosition) or (creature:getStorageValue(storageDive) > 0))
        and creature:getStorageValue(storageSurf) > 0 then
