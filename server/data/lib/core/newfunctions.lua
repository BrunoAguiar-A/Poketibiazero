...
function getBallKey(uid)
	if getItemBallKey then
		return getItemBallKey(uid)
	end

	local itemId = type(uid) == "userdata" and uid:getId() or uid
	...
	if type(uid) == "userdata" then
		local storedKey = uid:getSpecialAttribute("pokeBallKey")
		if storedKey and balls[storedKey] then
			return storedKey
		end
	end
...
local function getPokemonBallAndMaster(creature)
	...
	if usingBall and usingBall:getTopParent() == master and usingBall:getSpecialAttribute("isBeingUsed") == 1 then
		ball = usingBall
	elseif master.getPokeballsCached then
		for _, cachedBall in ipairs(master:getPokeballsCached()) do
			if cachedBall and cachedBall:getTopParent() == master and cachedBall:getSpecialAttribute("isBeingUsed") == 1 then
				ball = cachedBall
				master:setUsingPokeball(cachedBall)
				break
			end
		end
	end
...
local function ensurePokemonLevelData(ball, pokeName, options)
	if PokemonLevel and PokemonLevel.ensureBall then
		return PokemonLevel.ensureBall(ball, pokeName, options)
	end
	return false
end

local function getPokemonLevelScale(ball, pokeName, statName, monsterType)
	if PokemonLevel and PokemonLevel.getStatScale then
		return PokemonLevel.getStatScale(ball, pokeName, statName, monsterType)
	end
	return 1.0
end
...
function Monster.getTotalHealth(self)
	...
	if self:isPokemon() then
		...
		local ball, master = getPokemonBallAndMaster(self)
		if not ball or not master then return total end

		local pokeName = ball:getSpecialAttribute("pokeName") or self:getName()
		ensurePokemonLevelData(ball, pokeName)
		...
		total = total + bonusBoost
		total = total * getPokemonLevelScale(ball, pokeName, "hp", monsterType)
...
function MonsterType.getTotalHealth(self, ball, master)
	...
	if ball then
		...
		total = total + bonusBoost
		total = total * getPokemonLevelScale(ball, pokeName, "hp", self)
	end

	local guild = master and master:getGuild()
	if guild then
		local pokemonHealthBuff = guild:hasBuff(COLUMN_6, POKEMON_HEALTH_BUFF)
		if pokemonHealthBuff then
			total = total + (total * GUILD_BUFF_POKEMON_HEALTH / 100)
...
function Monster.getTotalMeleeAttack(self, cid)
	local bonus = bonus or 1 
	local monsterType = MonsterType(self:getName())
	if not monsterType then return 0 end
	if self:isPokemon() then
		local total = math.floor(monsterType:moveMagicAttackBase())
		local ball, master = getPokemonBallAndMaster(self)
		if not ball or not master then return 0 end
		local pokeName = ball:getSpecialAttribute("pokeName") or self:getName()
		ensurePokemonLevelData(ball, pokeName)
		...
		total = total * getPokemonLevelScale(ball, pokeName, "atk", monsterType)
...
function Monster.getTotalMagicAttack(self)
	...
	if self:isPokemon() then
		...
		if star >= 1 then total = total * (1 +(0.2*star)) end
		if maestria >= 2 then total = total * 1.1 end
		total = total * getPokemonLevelScale(ball, pokeName, "spatk", monsterType)
...
local function getPokemonDefenseTotal(creature, statName)
	...
	if creature:isPokemon() then
		...
		local ball, master = getPokemonBallAndMaster(creature)
		if not ball or not master then return total end
		local pokeName = ball:getSpecialAttribute("pokeName") or creature:getName()
		ensurePokemonLevelData(ball, pokeName)
		local bonusTotal = 0
		...
		total = total + bonusTotal
		total = total * getPokemonLevelScale(ball, pokeName, statName, monsterType)
...
function Monster.getTotalPhysicalDefense(self)
	return getPokemonDefenseTotal(self, "def")
end

function Monster.getTotalSpecialDefense(self)
	return getPokemonDefenseTotal(self, "spdef")
end
...
function Monster.getTotalDefense(self)
	...
	if not monsterType then return 0 end
	if self:isPokemon() then
		return (self:getTotalPhysicalDefense() + self:getTotalSpecialDefense()) / 2
	elseif self:isMonster() then
		return math.floor(monsterType:moveMagicDefenseBase())
	end
	return 0
...
function Monster.getTotalSpeed(self)
	...
	if self:isPokemon() then
		...
		local ball = select(1, getPokemonBallAndMaster(self))
		local bonusTotal = 0
		if ball then
			local pokeName = ball:getSpecialAttribute("pokeName") or self:getName()
			ensurePokemonLevelData(ball, pokeName)
			...
			total = total * getPokemonLevelScale(ball, pokeName, "speed", monsterType)
		end
		total = total + bonusTotal
		return total
	elseif self:isMonster() then
...
function Item.getPokeLevel(self)
	if PokemonLevel and PokemonLevel.ensureBall and self:isPokeball() then
		PokemonLevel.ensureBall(self, self:getSpecialAttribute("pokeName"))
		return PokemonLevel.getLevel(self)
	end
...
function Player.getSummonLevelFromBall(self)
	...
	if item then
		local pokeName = item:getSpecialAttribute("pokeName")
		ensurePokemonLevelData(item, pokeName)
		return PokemonLevel and PokemonLevel.getLevel and PokemonLevel.getLevel(item) or (item:getSpecialAttribute("pokeLevel") or 1)
	end
	return 1
end
...
function Monster.getSummonLevel(self)
	if self:isPokemon() then
		local item = select(1, getPokemonBallAndMaster(self))
...
function doReleaseSummon(cid, pos, effect, message, missile)
	...
	ensurePokemonLevelData(ball, name)
	...
	local summonLevel = PokemonLevel and PokemonLevel.getLevel and PokemonLevel.getLevel(ball) or (ball:getSpecialAttribute("pokeLevel") or 1)
	...
	if monster ~= nil then
		...
		local visualBoost = ball:getSpecialAttribute("pokeBoost") or 0
		local overheadName = PokemonLevel and PokemonLevel.formatOverheadName and PokemonLevel.formatOverheadName(name, summonLevel, visualBoost, true)
			or (visualBoost >= 1 and string.format("%s [%d +%d]", name, summonLevel, visualBoost) or string.format("%s [%d]", name, summonLevel))
		monster:setNickname(overheadName)
...
function Player:addPokemon(pokeName, ballId, gender, level, stars, rank, options)
	local ballKey
	if not ballId then
		ballKey = balls.pokeball
	else
		ballKey = KEYS_SEARCH[ballId] or balls.pokeball
	end
	...
	if not sendPokeballToDepot then
		pokeball = self:addItem((ballKey and ballKey.usedOn) or balls.pokeball.usedOn, 1, false)
	else
		local depot = self:getInbox()
		pokeball = depot.addItem and depot:addItem((ballKey and ballKey.usedOn) or balls.pokeball.usedOn, 1, INDEX_WHEREEVER, FLAG_NOLIMIT) or nil
	end
	if not pokeball then return error(("Could not add pokeball %s to player %s."):format(pokeName, self:getName())) end
	rememberSpeciesBallKey(pokeball, ballKey and ballKey.key or nil)
	...
	if stars then
		pokeball:setSpecialAttribute("pokeStars", stars)
	end
	if rank then
		pokeball:setSpecialAttribute("pokeRank", rank)
	end
	if PokemonLevel and PokemonLevel.initializeBall then
		PokemonLevel.initializeBall(pokeball, pokeName, {
			level = tonumber(level) or PokemonLevel.CONFIG.defaultLevel,
			generateBL = not (options and options.generateBL == false),
		})
	else
		pokeball:setSpecialAttribute("pokeLevel", tonumber(level) or 1)
	end
	local maxHealth = monsterType.getTotalHealth and monsterType:getTotalHealth(pokeball, self) or monsterType:getMaxHealth()
	pokeball:setSpecialAttribute("pokeMaxHealth", maxHealth)
	pokeball:setSpecialAttribute("pokeHealth", maxHealth)
	applyPokeballStateVisual(pokeball, "stored")
...
local function getPlayerFromVariant(player)
...
function doAddPokeball(player, pokeName, ballId)
	local targetPlayer = getPlayerFromVariant(player)
	if not targetPlayer then
		return nil
	end
	return targetPlayer:addPokemon(pokeName, ballId)
end

function doAddPokeballSupreme(player, pokeName, ballId)
	local pokeball = doAddPokeball(player, pokeName, ballId)
	if pokeball then
		pokeball:setSpecialAttribute("pokePower", "supreme")
	end
	return pokeball
end

function doAddPokeballFull(player, pokeName, ballId)
	local pokeball = doAddPokeball(player, pokeName, ballId)
	if pokeball then
		pokeball:setSpecialAttribute("pokePower", "full")
	end
	return pokeball
end
...
local function isFlyFloorNeeded(tile)
	-- Returns true only for sky tiles (no tile, or tile with no ground).
	-- Water tiles and normal ground tiles are handled by C++ fly logic and
	-- do NOT need a flyFloor item placed on them.
	if not tile or not tile:getGround() then
		return true
	end
	-- Do not place flyFloor on water tiles; the C++ queryAdd fly bypass covers them.
	if isInArray(waterIds, tile:getGround():getId()) then
		return false
	end
	-- Tile has a non-water ground: no flyFloor needed (player walks on the real ground).
	return false
end

function Position:createFlyFloor()
	local toTile = Tile(self)
	if isFlyFloorNeeded(toTile) then
		...
		for y = -1, 1 do
			for x = -1, 1 do
				...
				if isFlyFloorNeeded(toTileN) then
