...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (!g_game.placeCreature(player, player->getLoginPosition(), false, false, true)) {
...
void ProtocolGame::logout(bool displayEffect, bool forced)
{
	...
	if (!player->isRemoved()) {
		...
		if (player->isSpectator()) {
			player->setSpectating(nullptr);
		}

		for (Player* spectator : player->spectators) {
			spectator->setSpectating(nullptr);
		}
...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	static std::list<uint8_t> acceptedPackets = {
		0x14, 0x96, 0x97, 0x98, 0x99, 0x9A, 0x1E, 0x1D, 0x32
	};

	if (player->spectating && 
	    (std::find(acceptedPackets.begin(), acceptedPackets.end(), recvbyte) == acceptedPackets.end())) {
	    return;
	}
	...
	switch (recvbyte) {
		...
		default:
			g_dispatcher.addTask(createTask(std::bind(&Game::parsePlayerNetworkMessage, 
									&g_game, player->getID(), recvbyte, new NetworkMessage(msg))));
			break;
	}
...
bool ProtocolGame::canSee(int32_t x, int32_t y, int32_t z) const
{
	...
	const Position& myPos = player->isSpectator() ? player->getSpectating()->getPosition() : player->getPosition();
	if (myPos.z <= 7) {
...
void ProtocolGame::parseUseItem(NetworkMessage& msg)
{
	...
	player->setMarketSellItem(nullptr);
...
void ProtocolGame::sendCreatureNameColor(uint32_t id, NameColor_t color)
{
...
void ProtocolGame::sendJump(uint32_t id, uint32_t height, uint32_t duration)
{
    NetworkMessage msg;
    msg.addByte(0x44);
    msg.add<uint32_t>(id);
...
void ProtocolGame::sendAnimatedMessage(const TextMessage& message, bool isCritical)
{
	NetworkMessage msg;
	msg.addByte(0x84); //132
	msg.addPosition(message.position);
	msg.addByte(message.primary.color);
	msg.addString(message.text);
	if (isCritical) {
...
void ProtocolGame::setFly(bool fly)
{
	NetworkMessage msg;
	msg.addByte(0x4F);
	uint8_t flyByte = fly ? 0x01 : 0x00;
	msg.add<uint8_t>(flyByte);
...
void ProtocolGame::sendMapDescription(const Position& pos)
{
	NetworkMessage msg;
	msg.addByte(0x64);
	msg.addPosition(player->isSpectator() ? player->getSpectating()->getPosition() : player->getPosition());
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
	...
	if (creature != player) {
		if (stackpos != -1) {
			...
			for (Player* spectator : player->spectators) {
				if (spectator->client) {
					spectator->client->sendAddCreature(creature, pos, stackpos, isLogin);
				}
			}
		}
...
{
	auto tempPlayer = player->getSpectating() ? player->getSpectating() : player;
	if (creature == tempPlayer) {
		if (teleport) {
			...
			for (Player* spectator : player->spectators) {
				if (spectator->client) {
					spectator->client->sendMoveCreature(creature, newPos, newStackPos, oldPos, oldStackPos, teleport);
				}
			}
		}
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	NameColor_t creatureColorName = creature->nameColor;
	...
	if (known) {
		...
		if (monster) {
			isPoke = true;
			const uint32_t pokeLevel = std::max<uint32_t>(1, monster->getLevel());
			const Creature* master = monster->getMaster();
			const bool playerOwned = master && master->getPlayer();
			if (playerOwned) {
				const uint32_t boost = monster->getBoost();
				if (boost >= 1) {
					msg.addString(monster->getName() + " [" + std::to_string(pokeLevel) + " +" + std::to_string(boost) + "]");
				} else {
					msg.addString(monster->getName() + " [" + std::to_string(pokeLevel) + "]");
				}
			} else {
				msg.addString(monster->getName() + " [" + std::to_string(pokeLevel) + "]");
			}
	...
	msg.addByte(player->canWalkthroughEx(creature) ? 0x00 : 0x01);
	msg.add<uint32_t>(isPoke);
	msg.addString(nickname);
	
	const auto& iconMap = creature->getIconMap();
	msg.addByte(iconMap.size());
	for (const auto& entry : iconMap) {
    	msg.addString(entry.first);
    	msg.add<uint16_t>(entry.second.first);
    	msg.add<uint16_t>(entry.second.second);
	}
	
	msg.addString(creature->getTitle());
	msg.addString(creature->getTitleFont());
    msg.addString(creature->getTitleColor());

	if (monster) {
		msg.addByte(0x01);
		msg.addByte(monster->isGhost() ? 0x01 : 0x00);
	} else {
		msg.addByte(0x00);
	}

	const auto& attachedEffects = creature->getAttachedEffects();
	msg.addByte(attachedEffects.size());
	for (const auto& entry : attachedEffects) {
    	msg.add<uint16_t>(entry.first);
    	msg.addByte(entry.second);
	}
}
...
void ProtocolGame::AddPlayerStats(NetworkMessage& msg)
{
	...
	// msg.add<uint16_t>(std::min<int64_t>(player->getPlayerInfo(PLAYERINFO_MAXHEALTH), std::numeric_limits<uint16_t>::max()));
...
void ProtocolGame::parseExtendedOpcode(NetworkMessage& msg)
{
	...
	if (player->isSpectator() && opcode != 0x72)
		return;
...
void ProtocolGame::shakeScreen(uint32_t intensity, uint32_t duration)
{
    NetworkMessage msg;
    msg.addByte(0x41); 
    msg.add<uint32_t>(std::max(intensity, static_cast<uint32_t>(1)));
    msg.add<uint32_t>(std::max(duration, static_cast<uint32_t>(50)));
	writeToOutputBuffer(msg);
...
void ProtocolGame::sendStartSpectating(Player* target)
{
	sendMapDescription(target->getPosition());
	for (int i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; ++i) {
		sendInventoryItem(static_cast<slots_t>(i), target->getInventoryItem(static_cast<slots_t>(i)));
	}
	target->sendIcons();
	target->sendStats();
	target->sendSkills();
...
void ProtocolGame::sendStopSpectating()
{
