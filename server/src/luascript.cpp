...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMethod("Player", "refreshWalkCache", LuaScriptInterface::luaPlayerRefreshWalkCache);
...
int LuaScriptInterface::luaMonsterSetLevel(lua_State* L)
{
	...
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		monster->setLevel(getNumber<uint32_t>(L, 2));
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerRefreshWalkCache(lua_State* L)
{
	// player:refreshWalkCache()
	// Rebuilds the walk cache to reflect the player's current state (e.g. after fly is activated).
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->updateMapCache();
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
