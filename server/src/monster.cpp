...
Monster* Monster::createMonster(const std::string& name, uint32_t lvl, uint32_t bst) //pota
{
...
Monster::Monster(MonsterType* mType, uint32_t lvl, uint32_t bst) : //pota
	Creature(),
	strDescription(mType->nameDescription),
	mType(mType)
{

	health = mType->info.wildHealth;
	healthMax = mType->info.wildMaxHealth;
	baseSpeed = mType->info.wildBaseSpeed;
	level = lvl > 0 ? lvl : std::max<int32_t>(1, mType->info.minimumLevel);
	boost = bst;
...
{
	//std::cout << "CreatureName " << creature->getName() << " on MonsterName Screen: " << getName() << std::endl;
                             ...
                             const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if (creature == this) {
		...
		if (master && isMonsterGuardian()) {
			const Position& pos = master->getPosition();
				if (Position::getDistanceZ(newPos, pos) > 0 || 
					(std::max<int32_t>(Position::getDistanceX(newPos, pos), Position::getDistanceX(newPos, pos)) > 11) || 
						(std::max<int32_t>(Position::getDistanceY(newPos, pos), Position::getDistanceY(newPos, pos)) > 7)) {
				Tile* destTile = g_game.map.getTile(pos);
				g_game.map.moveCreature(*this, *destTile);
...
bool Monster::isOpponent(const Creature* creature) const
{
	...
	if (creature->getSummonCount() > 0 || creature->isMonsterGuardian()) {
		return false;
	}

	if (getMaster() && getMaster()->getDuelist()) {  
	    Duelist* masterDuelist = getMaster()->getDuelist();

	    if (masterDuelist) {
	        if (masterDuelist->getContractPlayer() == 0) {
	            return true;
	        } else {
	            if (masterDuelist->getContractPlayer() > 0 && ((creature->getMaster() && creature->getMaster()->getPlayer()) || creature->getPlayer())) {
	                const Player* player = creature->getPlayer() ? creature->getPlayer() : creature->getMaster()->getPlayer();
	
	                if (player->getGUID() == masterDuelist->getContractPlayer()) { 
	                    return true; 
	                }
	            }
	        }
	...
	if (isSummon() && getMaster()->getPlayer()) {
		if ((creature->getMaster() && creature->getMaster() == getMaster())) {
			return false;
		}
...
void Monster::doAttacking(uint32_t interval)
{
	...
	for (const spellBlock_t& spellBlock : isSummon() && getMaster()->getPlayer() && !isMonsterGuardian() ? mType->info.moves : mType->info.attackSpells) {
		bool inRange = false;
                          ...
                          const spellBlock_t& sb, uint32_t interval, bool& inRange, bool& resetTicks)
{
	...
	if (isSummon() && getMaster()->getPlayer() && !sb.isMelee && !sb.isPassive && !isMonsterGuardian()) { // summons não podem soltar magias alem de: melee / passivas
		return false;
	}

	if (hasCondition(CONDITION_SLEEP) || hasCondition(CONDITION_STUN) || hasCondition(CONDITION_SILENCE) || hasCondition(CONDITION_FEAR) ) {
		return false;
	}
...
void Monster::onThinkTarget(uint32_t interval)
{
	...
	} else if (isMonsterGuardian()) {
		if (!attackedCreature) {
			// g_game.internalCreatureSay(this, TALKTYPE_MONSTER_YELL, "Target Think!", false);
			searchTarget(TARGETSEARCH_RANDOM);
		}
	}
}
...
void Monster::death(Creature*)
{
	...
	if (effectId != 0) {
		g_game.addMagicEffect(getPosition(), effectId);
	}
...
bool Monster::challengeCreature(Creature* creature)
{
	...
	if (!result && isHostile() && isTarget(creature)) {
		result = setAttackedCreature(creature);
...
bool Monster::convinceCreature(Creature* creature)
{
	...
	if (Creature* target = creature->getAttackedCreature() ) {
...
{
	if (const Creature* master = getMaster()) { // refazer no futuro.
	    Player* player = const_cast<Player*>(master->getPlayer());
	    if (player) {
	        Item* pokeball = player->getUsingPokeball();
	        if (pokeball && pokeball->getIntAttr(ITEM_ATTRIBUTE_HELDU) == 253) {
	            return true;
	        }
	    }
	}

	return ((mType->info.race == RACE_GHOST) || (mType->info.race2 == RACE_GHOST));
}
