...
void Tile::setZoneId(uint16_t id, bool isLoadedFromMap) {
    zoneId = id;
    if (isLoadedFromMap) {
        Zone* zone = g_game.getZoneById(id);
		if(zone) {
        	zone->addTile(this);
		}
    }
...
ReturnValue Tile::queryAdd(int32_t, const Thing& thing, uint32_t, uint32_t flags, Creature*) const
{
	if (const Creature* creature = thing.getCreature()) {
		...
		if (creature->getMonster() && creature->getMonster()->isGhost()){
			 if ((creature->isSummon() && !hasFlag(TILESTATE_PROTECTIONZONE) && !creature->getMaster()->getPlayer()) || 
			 		(creature->isSummon() && creature->getMaster()->getPlayer() && !hasFlag(TILESTATE_FLOORCHANGE | TILESTATE_TELEPORT))) {	
				return RETURNVALUE_NOERROR;
			 }
		}
		...
		if ((ground->getID() == 460 && !creature->getPlayer())){
			return RETURNVALUE_NOTPOSSIBLE;
		}

		if (creature->getDuelist()) {
			if (hasFlag(TILESTATE_PROTECTIONZONE | TILESTATE_FLOORCHANGE | TILESTATE_TELEPORT))
				return RETURNVALUE_NOTPOSSIBLE;
		}

		if (const Monster* monster = creature->getMonster()) {
			if (hasFlag(TILESTATE_PROTECTIONZONE | TILESTATE_FLOORCHANGE | TILESTATE_TELEPORT) && (!monster->isSummon() || (monster->isSummon() && monster->getMaster() && monster->getMaster()->getAttackedCreature()))) {
				return RETURNVALUE_NOTPOSSIBLE;
			}
		...
		if (!hasBitSet(FLAG_IGNOREBLOCKITEM, flags)) {
			...
			if (hasFlag(TILESTATE_BLOCKSOLID)) {
				// Flying players can walk over ground-only solid tiles (water, etc.)
				// but are still blocked if any placed item on the tile is solid
				const Player* player = creature->getPlayer();
				int32_t flyValue = 0;
				if (player) player->getStorageValue(5001, flyValue);
				if (flyValue == 1) {
					bool itemsBlock = false;
					...
					// Only ground is blockSolid — flying player passes over it
				} else {
					return RETURNVALUE_NOTENOUGHROOM;
				}
			}
