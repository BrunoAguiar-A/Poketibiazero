...
Creature* Game::getCreatureByID(uint32_t id)
{
	...
	else if (id <= Duelist::duelistAutoID) {
		return getDuelistByID(id);
	}
	return nullptr;
}
...
Duelist* Game::getDuelistByID(uint32_t id)
{
	if (id == 0) {
		return nullptr;
	}

	auto it = duelists.find(id);
	if (it == duelists.end()) {
		return nullptr;
	}
	return it->second;
...
bool Game::internalPlaceCreature(Creature* creature, const Position& pos, bool extendedPos /*=false*/, bool forced /*= false*/, bool isLogin /*= false*/)
{
	if (creature->getParent() != nullptr) {
		return false;
	}

	if (!map.placeCreature(pos, creature, extendedPos, forced, isLogin)) {
		return false;
	}
...
bool Game::placeCreature(Creature* creature, const Position& pos, bool extendedPos /*=false*/, bool forced /*= false*/, bool isLogin /*= false*/)
{
	if (!internalPlaceCreature(creature, pos, extendedPos, forced, isLogin)) {
		return false;
	}
...
void Game::startJump(Creature* creature, uint32_t height, uint32_t duration)
{
	SpectatorVec spectators;
	map.getSpectators(spectators, creature->getPosition(), false, true);
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			tmpPlayer->sendJump(creature->getID(), height, duration);
		}
	}
}

void Game::updateCreatureNameColor(const Creature* creature, NameColor_t color)
{
	SpectatorVec spectators;
	map.getSpectators(spectators, creature->getPosition(), false, true);
	...
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			tmpPlayer->sendCreatureNameColor(creature->getID(), color);
		}
	}
}
...
{
	if (creature->getMonster() && creature->getMonster()->isPokeStopped()) {
	...
	Tile* currentTile = map.getTile(currentPos.x, currentPos.y, currentPos.getZ());
	Tile* nextTile = map.getTile(destPos.x, destPos.y, destPos.z);

	if (creature && currentTile && nextTile && (currentTile->getZoneId() != nextTile->getZoneId())) {
		Zone* oldZone = getZoneById(currentTile->getZoneId());
		Zone* newZone = getZoneById(nextTile->getZoneId());

		if (!g_events->eventCreatureOnChangeZone(creature, oldZone, newZone, false)) {
			return RETURNVALUE_NOTPOSSIBLE;
		}
	}
	...
	if (player && toTile->getGround() && toTile->getGround()->getID() == 460) {
		int32_t value;
		player->getStorageValue(5001, value);
		if (value != 1) {
			return RETURNVALUE_NOTPOSSIBLE;
		}
	}
...
{
	if (!player->canDoAction() && player->getAccountType() < ACCOUNT_TYPE_GOD) {
		uint32_t delay = player->getNextActionTime();
	...
	if (!Position::areInRange<1, 1>(playerPos, mapFromPos) && player->getAccountType() < ACCOUNT_TYPE_GOD) {
	...
	if (item->isHangable() && toCylinderTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
		...
		if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos)) {
			...
			std::forward_list<Direction> listDir;
			if (player->getPathTo(walkPos, listDir, 0, 0, true, true) && player->getAccountType() < ACCOUNT_TYPE_GOD) {
				g_dispatcher.addTask(createTask(std::bind(&Game::playerAutoWalk,
					this, player->getID(), listDir)));
	...
	if ((Position::getDistanceX(playerPos, mapToPos) > item->getThrowRange() && player->getAccountType() < ACCOUNT_TYPE_GOD) ||
		(Position::getDistanceY(playerPos, mapToPos) > item->getThrowRange() && player->getAccountType() < ACCOUNT_TYPE_GOD) ||
		(Position::getDistanceZ(mapFromPos, mapToPos) * 4 > item->getThrowRange() && player->getAccountType() < ACCOUNT_TYPE_GOD)) {
		player->sendCancelMessage(RETURNVALUE_DESTINATIONOUTOFREACH);
		return;
	}
	...
	Item* item, uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Creature* actor/* = nullptr*/, Item* tradeItem/* = nullptr*/)
{
	...
	if (item->isStackable()) {
		...
		if (item->equals(toItem)) {
			n = std::min<uint32_t>(MAX_ITEM_COUNT - toItem->getItemCount(), m);
	...
	uint32_t flags, bool test, uint32_t& remainderCount)
{
	...
	*/
	...
	if (item->isStackable() && item->equals(toItem)) {
		...
		uint32_t n = std::min<uint32_t>(MAX_ITEM_COUNT - toItem->getItemCount(), m);
...
ReturnValue Game::internalTeleport(Thing* thing, const Position& newPos, bool pushMove/* = true*/, uint32_t flags /*= 0*/)
{
	...
	if (Creature* creature = thing->getCreature()) {
		...
		if (creature && currentTile && toTile && (currentTile->getZoneId() != toTile->getZoneId())) {
			Zone* oldZone = getZoneById(currentTile->getZoneId());
			Zone* newZone = getZoneById(toTile->getZoneId());

			if (!g_events->eventCreatureOnChangeZone(creature, oldZone, newZone, true)) {
				return RETURNVALUE_NOTPOSSIBLE;
			}
		}
...
{
	if (amount == 0 || amount > MAX_ITEM_COUNT) {
		return;
	}
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if (Monster* attackedMonster = attackCreature->getMonster()) {
		if (!attackedMonster->getMaster()) {
			Creature* retaliationTarget = player;
			const std::list<Creature*>& summons = player->getSummons();
			if (!summons.empty()) {
				retaliationTarget = summons.front();
			}
			if (retaliationTarget != attackCreature) {
				attackedMonster->challengeCreature(retaliationTarget);
			}
		}
	}
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, [[maybe_unused]] bool checkDefense, [[maybe_unused]] bool checkArmor, [[maybe_unused]] bool field)
{
	if (damage.primary.type == COMBAT_NONE) {
		return true;
	}

	if (damage.primary.type == COMBAT_HEALING && attacker != target) {
		Monster* monster = target->getMonster();
		if (monster && monster->isTarget(attacker))
			return true;
	}
...
void Game::combatGetTypeInfo(CombatType_t combatType, [[maybe_unused]] Creature* target, TextColor_t& color, uint8_t& effect)
{
	switch (combatType) {
	...
	case COMBAT_EARTHDAMAGE: {
		color = TEXTCOLOR_EARTH_DAMAGE;
		effect = CONST_ME_NONE;
		break;
	}

	case COMBAT_POISONDAMAGE: {
		color = TEXTCOLOR_POISON;
	...
	case COMBAT_PSYCHICDAMAGE: {
		color = TEXTCOLOR_PSYCHIC;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_GRASSDAMAGE: {
		color = TEXTCOLOR_GRASS;
	...
	case COMBAT_WATERDAMAGE: {
		color = TEXTCOLOR_WATER;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_FLYINGDAMAGE: {
		color = TEXTCOLOR_FLYING;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_ELECTRICDAMAGE: {
		color = TEXTCOLOR_ELECTRIC;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_GROUNDDAMAGE: {
		color = TEXTCOLOR_GROUND;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_ROCKDAMAGE: {
		color = TEXTCOLOR_ROCK;
		effect = CONST_ME_NONE;
		break;
	}
	...
	case COMBAT_DRAGONDAMAGE: {
		color = TEXTCOLOR_DRAGON;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_GHOSTDAMAGE: {
		color = TEXTCOLOR_GHOST;
		effect = CONST_ME_NONE;
		break;
	}
	...
	case COMBAT_STEELDAMAGE: {
		color = TEXTCOLOR_STEEL;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_FAIRYDAMAGE: {
		color = TEXTCOLOR_FAIRY;
		effect = CONST_ME_NONE;
		break;
	}
	case COMBAT_FIGHTINGDAMAGE: {
		color = TEXTCOLOR_FIGHTING;
		effect = CONST_ME_NONE;
		break;
	}
	...
	if (damage.primary.value > 0) {
		...
		if (attacker) {
			...
			if (damage.origin != ORIGIN_RETURN) {
				...
				MonsterType* monsterType = g_monsters.getMonsterType(pokemon->getName());
				if (monsterType) {
					if (pokemon->getMaster() && pokemon->getMaster()->getPlayer() && !pokemon->isMonsterGuardian())
						isWild = false;
			...
			if (pokemon->hasCondition(CONDITION_RAGE)) {
				damage.primary.value *= 2;
			}
		}

		int64_t healthChange = damage.primary.value;
		if (healthChange == 0) {
			return true;
		}
			...
			if (manaDamage != 0) {
				if (damage.origin != ORIGIN_NONE) {
					...
					if (!events.empty()) {
						...
						healthChange = damage.primary.value;
						if (healthChange == 0) {
							return true;
						}
		...
		int64_t realDamage = damage.primary.value;
		if (realDamage == 0) {
			return true;
		}
		...
		realDamage = damage.primary.value;
		if (realDamage == 0) {
			return true;
		}
		...
		if (message.primary.value) {
			...
			if (damage.critical) {
				message.primary.color = TEXTCOLOR_CRITICAL;
			}
			if (hitEffect != CONST_ME_NONE) {
				addMagicEffect(spectators, targetPos, hitEffect);
			}
			for (Creature* spectator : spectators) {
				Player* playerSpectator = spectator->getPlayer();
				if (playerSpectator) {
					playerSpectator->sendAnimatedText(message, damage.critical);
				}
			}
		}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	int64_t manaChange = damage.primary.value;
	if (manaChange > 0) {
		if (attacker) {
...
void Game::internalDecayItem(Item* item)
{
	...
	if (it.decayTo != 0) {
		...
		//std::cout << "[Debug - Game::internalDecayItem] item id: " << item->getID() << std::endl;
		startDecay(newItem);
	}
	else {
...
/*void Game::sendCreatureIcons(Creature* creature) {
	SpectatorVec spectators;
	map.getSpectators(spectators, creature->getPosition(), true);

	for (Creature* spectator : spectators) {
		spectator->getPlayer()->sendCreatureIcons(creature);
	}
}
*/
...
bool Game::loadZones()
{
	...
	pugi::xml_document doc;
	pugi::xml_parse_result result = doc.load_file("data/XML/zones.xml");
	if (!result) {
		printXMLError("Error - Zones::loadFromXml", "data/XML/zones.xml", result);
		return false;
	}
	else {

		for (pugi::xml_node xmlZone : doc.child("zones").children("zone")) {
			uint16_t zoneId = xmlZone.attribute("id").as_uint();
			std::string zoneName = xmlZone.attribute("name").as_string();
			std::vector<zoneInfos> positions;

			for (pugi::xml_node xmlArea : xmlZone.children("area")) {
				zoneInfos posInfo;

				posInfo.topleft.x = xmlArea.child("topleft").attribute("x").as_int();
				posInfo.topleft.y = xmlArea.child("topleft").attribute("y").as_int();
				posInfo.topleft.z = xmlArea.child("topleft").attribute("z").as_int();

				posInfo.bottomright.x = xmlArea.child("bottomright").attribute("x").as_int();
				posInfo.bottomright.y = xmlArea.child("bottomright").attribute("y").as_int();
				posInfo.bottomright.z = xmlArea.child("bottomright").attribute("z").as_int();

				positions.push_back(posInfo);
			}
			g_game.addZone(zoneId, positions, zoneName);
		}
	}

	std::cout << ">> Zones loaded in: " << (OTSYS_TIME() - start) / (1000.) << " seconds <<" << std::endl;
	return true;
}
...
void Game::addDuelist(Duelist* duelist)
{
	duelists[duelist->getID()] = duelist;
}

void Game::removeDuelist(Duelist* duelist)
{
	duelists.erase(duelist->getID());
}
...
bool Game::reload(ReloadTypes_t reloadType)
{
	...
	case RELOAD_TYPE_STAGE: return loadExperienceStages();
	...
	case RELOAD_TYPE_OUTFITS: return outfits.reload();
...
std::string Game::humanCase(const std::string& input) {
	...
	words.push_back(input.substr(start)); // Última palavra
	// Capitalizar a primeira letra de cada palavra
	for (std::string& word : words) {
		if (!word.empty()) {
			word[0] = std::toupper(word[0]); // Capitalizar primeira letra
			for (size_t i = 1; i < word.size(); ++i) {
				word[i] = std::tolower(word[i]); // Converter o restante para minúsculas
			}
		}
	}
	...
	for (const std::string& word : words) {
		if (!result.empty()) {
			result += " "; // Adicionar espaço entre as palavras
		}
		result += word;
	}
	return result;
...
{
	Database& db = Database::getInstance();
	if (DBResult_ptr result = db.storeQuery("SELECT `guilds`.*, `players`.name AS leader FROM `guilds` INNER JOIN `players` INNER JOIN `guild_members` AS `gm` ON `players`.id = `gm`.player_id AND `gm`.leader = 1 AND `gm`.guild_id = `guilds`.id;")) {
		do {
			uint32_t guildId = result->getNumber<uint32_t>("id");
			Guild* guild = new Guild(guildId, result->getString("name"));
			guild->setLevel(result->getNumber<uint16_t>("level"));
			guild->setGold(result->getNumber<uint64_t>("gold"));
			guild->setRequiredLevel(result->getNumber<uint32_t>("required_level"));
			guild->setEmblem(result->getNumber<uint16_t>("emblem"));
			guild->setWarsWon(result->getNumber<uint16_t>("wars_won"));
			guild->setWarsLost(result->getNumber<uint16_t>("wars_lost"));
			guild->setLanguage(result->getNumber<uint16_t>("language"));
			guild->setJoinStatus(result->getNumber<uint16_t>("join_status"));
			guild->setMotd(std::move(result->getString("motd")));
			guild->setLeaderName(std::move(result->getString("leader")));
			guild->setPacifism(result->getNumber<int64_t>("pacifism"));
			guild->setPacifismStatus(result->getNumber<uint16_t>("pacifism_status"));
			guild->setLastBuffSave(result->getNumber<uint64_t>("buffs_save"));
			guild->getCombinedLevels();

			unsigned long attrSize;
			PropStream propStream;
			const char* attr = result->getStream("buffs", attrSize);
			propStream.init(attr, attrSize);

			uint8_t row = 0;
			GuildBuffType type;
			while (propStream.read<GuildBuffType>(type)) {
				guild->setBuff(row++, type);
			}

			if (DBResult_ptr result2 = db.storeQuery(fmt::format("SELECT `id`, `name`, `permissions`, `default`, `leader` FROM `guild_ranks` WHERE `guild_id` = {:d}", guildId))) {
				do {
					guild->addRank(result2->getNumber<uint32_t>("id"), result2->getString("name"), result2->getNumber<int32_t>("permissions"), result2->getNumber<uint16_t>("default"), result2->getNumber<uint16_t>("leader"));
				} while (result2->next());
			}

			if (DBResult_ptr result2 = db.storeQuery(fmt::format("SELECT `players`.id, `players`.name, `players`.vocation, `players`.level, `players`.lastlogin, `guild_members`.rank_id, `guild_members`.contribution FROM `guild_members` INNER JOIN `players` ON `guild_members`.player_id = `players`.id WHERE `guild_members`.guild_id = {:d}", guildId))) {
				do {
					guild->addMember(result2->getNumber<uint32_t>("id"), result2->getNumber<uint32_t>("vocation"), result2->getString("name"), guild->getRankById(result2->getNumber<uint32_t>("rank_id"))->name, result2->getNumber<uint32_t>("level"), result2->getNumber<uint64_t>("lastlogin"), result2->getNumber<uint64_t>("contribution"));
				} while (result2->next());
			}
