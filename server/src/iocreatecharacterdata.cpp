...
bool IOCreateCharacterData::insertCharacter(const Account account, const std::string& characterName, uint8_t playerSex, uint32_t connectionIp) {
    ...
    constexpr int32_t defaultTownId = 1;
    constexpr int32_t defaultPosX = 16254;
    constexpr int32_t defaultPosY = 16681;
    constexpr int32_t defaultPosZ = 6;

    if (db.executeQuery(fmt::format("INSERT INTO `players` SET `name` = {:s}, `account_id` = {:d}, `lastip` = {:d}, `sex` = {:d}, `creationdate` = {:d}, `lookType` = {:d}, `lookbody` = {:d}, `lookfeet` = {:d}, `lookhead` = {:d}, `looklegs` = {:d}, `level` = {:d}, `town_id` = {:d}, `posx` = {:d}, `posy` = {:d}, `posz` = {:d}", db.escapeString(name), account.id, connectionIp, playerSex, time(nullptr), lookType, lookbody, lookfeet, lookhead, looklegs, 8, defaultTownId, defaultPosX, defaultPosY, defaultPosZ))) {
