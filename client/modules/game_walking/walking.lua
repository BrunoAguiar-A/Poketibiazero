...
function isKeyboardWalkingBlocked()
...
function bindWalkKey(key, dir)
  ...
  g_keyboard.bindKeyDown(key, function()
    if isKeyboardWalkingBlocked() then return false end
    changeWalkDir(dir)
...
function bindTurnKey(key, dir)
  ...
  g_keyboard.bindKeyUp(key, function()
    if isKeyboardWalkingBlocked() then return false end
    local player = g_game.getLocalPlayer() if player then player:lockWalk(200) end
  end, gameRootPanel)
end
