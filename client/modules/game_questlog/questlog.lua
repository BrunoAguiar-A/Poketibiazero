...
function init()
  ...
  window.showButton.onClick = function()
  show()
end
    questLogButton = modules.client_topmenu.addLeftGameButton('questLogButton', tr('Quest Log'), '/images/TOPBUTTONS_REWORK/ICON_QUEST', function() g_game.requestQuestLog() end, false, 8)
...
function show(questlog)
  if questlog then
    -- Mostrando o QuestLog
    window:raise()
    window:show()
    window:focus()
    window.missionlog.currentQuest = nil -- resetando quest atual
    ...
    -- Mostra botão "Show" com texto 'Show'
    window.showButton:setVisible(true)
    window.showButton:setText('Show')

    -- Atribui função showQuestLine() ao botão
    window.showButton.onClick = function()
      modules.game_questlog.showQuestLine()
    end

    -- Reset da checkbox e descrição
    ...
    -- Mostrando o MissionLog
    window.questlog:setVisible(false)
    window.missionlog:setVisible(true)

    -- Botão de voltar
    -- window.closeButton:setText('<')

    -- Altera texto e função do botão "Show" para 'Back'
    window.showButton:setVisible(true)
    window.showButton:setText('Back')

    -- Atribui função de voltar ao QuestLog
    window.showButton.onClick = function()
      modules.game_questlog.back()
    end
  end
end
