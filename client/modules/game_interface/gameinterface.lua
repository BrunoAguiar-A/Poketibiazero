...
function init()
  ...
  logoutButton = modules.client_topmenu.addLeftButton('logoutButton', tr('Exit'),
    '/images/TOPBUTTONS_REWORK/ICON_LOGOUT', tryLogout, true)
...
function onUseWith(clickedWidget, mousePosition)
  ...
  if clickedWidget:getClassName() == 'UIGameMap' then
    ...
    if tile then
      local positionOffset = clickedWidget:getPositionOffset(mousePosition)
      if selectedThing:getId() == POKEDEX_ITEM_ID then
        local targetCreature = tile:getTopCreatureEx(positionOffset) or tile:getTopCreature()
        if targetCreature then
          g_game.useWith(selectedThing, targetCreature, selectedSubtype)
          return
        end
      end
      ...
      if selectedThing:isFluidContainer() or selectedThing:isMultiUse() then
        if selectedThing:getId() == 3180 or selectedThing:getId() == 3156 then
          ...
          g_game.useWith(selectedThing, tile:getTopMultiUseThingEx(positionOffset),
            selectedSubtype)
        end
      else
        g_game.useWith(selectedThing, tile:getTopUseThing(), selectedSubtype)
      ...
      if creatureThing:isNpc() then
        menu:addOption(tr('Talk'), function()
          ...
          if ((destPos.x >= myPos.x - 3) and (destPos.x <= myPos.x + 3) and (destPos.y >= myPos.y - 3) and (destPos.y <= myPos.y + 3)) then
            ...
            "Voce nao pode conversar com o npc, voce esta muito longe")
          end
        end, shortcut)
      end
