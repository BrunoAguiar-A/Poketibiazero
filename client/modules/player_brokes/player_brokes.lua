...
local function getDefaultBrokes()
  local t = {}
  for _, b in ipairs(balls_order) do
    if b ~= "master" then
      t[b] = 0
    end
  end
...
function ensurePokemonData(name, looktype, id)
  if not name or #name == 0 then return end
  if looktype then
    looktypes[name] = looktypes[name] or looktype
  end
  totalBrokes[name] = totalBrokes[name] or 0
  brokes[name] = brokes[name] or getDefaultBrokes()
  if not pontos[name] then
    pontos[name] = { total = 0, atual = 0 }
  end
  local exists = false
  for _, info in ipairs(dataCache) do
    if info.pokeName == name then
      exists = true
      break
    end
  end
  if not exists then
    table.insert(dataCache, { id = id or 0, pokeName = name })
...
function receiveOpcode(protocol, opcode, buffer)
  ...
  if type == "updateData" then
    local name = data.pokemonName
    totalBrokes[name] = (totalBrokes[name] or 0) + 1
    brokes[name] = brokes[name] or getDefaultBrokes()
    local ball = data.pokeball
    brokes[name][ball] = (brokes[name][ball] or 0) + 1
    local totalPrev = pontos[name] and pontos[name].total or (data.pontosTotais or 0)
    pontos[name] = pontos[name] or { total = totalPrev, atual = 0 }
    pontos[name].atual = (pontos[name].atual or 0) + (data.pontos or 0)
  end

  if type == "resetData" then
    local name = data.pokemonName
    totalBrokes[name] = 0
    brokes[name] = data.brokes or getDefaultBrokes()
    local totalPrev = pontos[name] and pontos[name].total or (data.pontosTotais or 0)
    pontos[name] = { total = totalPrev, atual = (data.pontos or 0) }
  end
end
...
function showPokeInfo(id)
  ...
  local key = id
  if not pontos[key] or not looktypes[key] then
    local lid = string.lower(id)
    for k,_ in pairs(pontos) do
      if string.lower(k) == lid then
        key = k
        break
      end
    end
  ...
  infoWindow.pokemonName:setText(key)
  infoWindow.pokemonLooktype:setOutfit({type = looktypes[key]})
  infoWindow.pokemonLooktype:setOldScaling(true)
  infoWindow.pontuacao:setText(string.format("%d/%d", pontos[key].atual, pontos[key].total))
  ...
  for index, ball in ipairs(balls_order) do
    if ball ~= "master" then
      ...
      if not TYPES_POINTS[types[key]][balls[ball]] and not TYPES_POINTS[types2[key]][balls[ball]] then
  ...
  for index, ball in ipairs(balls_order) do
    if ball ~= "master" then
      ...
      widget:setItemId(balls[ball])
      local brokes = tonumber(brokes[key][ball])
      if brokes >= 1000 then
