...
function g_advancedTooltip.display()
	...
	if currentAdvancedHoveredWidget:getClassName() == "UIItem" then
		...
		if item then
			...
			-- PRIORIDADE 1: Tooltip completo de Pokémon (containers/inventário)
			if currentAdvancedHoveredWidget.pokemonTooltipInfo then
				if not advancedTooltipPokemon then
					return
				end
				...
				local pokeBoost = getBallSpecialAttribute("pokeBoost", data) or 0
				...
				local rankName = getBallSpecialAttribute("rankName", data) or "F"
				...
				if advancedTooltipPokemon.pokeBoostLabel then
					advancedTooltipPokemon.pokeBoostLabel:setText("+" .. tostring(pokeBoost))
				end
				...
				local pokeHP = getBallSpecialAttribute("pokeHP", data)
				local pokeDamage = getBallSpecialAttribute("pokeDamage", data)
				local pokeSpeed = getBallSpecialAttribute("pokeSpeed", data)
				local pokeDefense = getBallSpecialAttribute("pokeDefense", data)
				local pokeBonus = getBallSpecialAttribute("pokeBonus", data)
				
				advancedTooltipPokemon.rank:setText(rankName or "-")
				advancedTooltipPokemon.bonus:setText(pokeBonus or "-")
				advancedTooltipPokemon.hp:setText(pokeHP or "-")
				advancedTooltipPokemon.boost:setText("+" .. tostring(pokeBoost))
				advancedTooltipPokemon.damage:setText(pokeDamage or "-")
				advancedTooltipPokemon.speed:setText(pokeSpeed or "-")
				advancedTooltipPokemon.defense:setText(pokeDefense or "-")
				
				-- Processar helds (mostrar nomes em texto)
				local heldsPanel = advancedTooltipPokemon.heldsPanel
				if heldsPanel then
					local heldx = getBallSpecialAttribute("heldx", data)
					local heldy = getBallSpecialAttribute("heldy", data)
					local heldu = getBallSpecialAttribute("heldu", data)
					
					-- Criar texto para cada held
					local heldsText = ""
					if heldx and heldx > 0 then
						heldsText = heldsText .. getHeldName(heldx)
					end
					if heldy and heldy > 0 then
						if heldsText ~= "" then heldsText = heldsText .. "\n" end
						heldsText = heldsText .. getHeldName(heldy)
					end
					if heldu and heldu > 0 then
						if heldsText ~= "" then heldsText = heldsText .. "\n" end
						heldsText = heldsText .. getHeldName(heldu)
					end
					
					-- Definir texto diretamente no painel (auto-resize)
					if heldsText ~= "" then
						heldsPanel:setText(heldsText)
						heldsPanel:setFont('verdana-11px-rounded')
						heldsPanel:setColor('#ffffff')
						heldsPanel:setTextAlign(AlignCenter)
					else
						heldsPanel:setText("None")
						heldsPanel:setFont('verdana-11px-rounded')
						heldsPanel:setColor('#999999')
						heldsPanel:setTextAlign(AlignCenter)
					end
				end

				advancedMovePokeToolTip(true)
			-- PRIORIDADE 2: Tooltip do market (simples, só nome e boost)
			elseif currentAdvancedHoveredWidget.pokeballsInfo then
			...
			-- PRIORIDADE 3: Tooltip de item normal
