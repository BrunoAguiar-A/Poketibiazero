...
function show()
  ...
  waitingEvent = scheduleEvent(function()
    if waitingPass then
      print("[PASS] Nenhum pacote de ExtendedOpcode "..Opcode.." recebido após abrir o passe. Verifique servidor: handler do /passopen e opcode "..Opcode..".")
    end
  end, 3000)
  -- Além do comando /passopen, envie um ping de opcode para servidores que esperam requisição explícita
  if g_game.isOnline() then
    g_game.getProtocolGame():sendExtendedOpcode(Opcode, "Open")
  end
...
function showBuyPassElite()
  print("[PASS] Abrindo janela de compra do Passe Elite (30 diamonds)")
...
function doBuyPass35Server()
  print("[PASS] Comprando Passe Elite...")
...
function doCloseBuyWindow()
  ...
  local confirmWindow50 = MainWindow:getChildById('UIBuyPass50')
  if confirmWindow50 then
    confirmWindow50:hide()
  end
end
...
function getPass(protocol, opcode, buffer)
  print("[PASS] Buffer recebido (primeiros 200 chars): " .. string.sub(buffer, 1, 200))
  ...
  local func, err = loadstring("return ".. buffer)
  if not func then
    print("[PASS] ERRO ao compilar buffer: " .. tostring(err))
    return
  end
  
  local success, receive = pcall(func)
  if not success then
    print("[PASS] ERRO ao executar buffer: " .. tostring(receive))
    return
  end
  
  if not receive then
    print("[PASS] ERRO: receive é nil!")
    return
  end
  
  print("[PASS] Receive[3]: " .. tostring(receive[3]))
  print("[PASS] Dump do receive:")
  dump(receive)
  ...
  elseif receive[3] == 'Items' then
    print("[PASS] Processando Items...")
    local first = Protocol_read(receive)
    local items = Protocol_read(receive)
    local collecteds = Protocol_read(receive)
    print("[PASS] First: " .. tostring(first))
    print("[PASS] Items type: " .. type(items))
    print("[PASS] Items count: " .. (items and #items or "nil"))
	if first then
  ...
  elseif receive[3] == 'Missions' then
    print("[PASS] Processando Missions...")
    local first = Protocol_read(receive)
    local missions = Protocol_read(receive)
    print("[PASS] First: " .. tostring(first))
    print("[PASS] Missions type: " .. type(missions))
    print("[PASS] Missions count: " .. (missions and #missions or "nil"))
	if first then
