...
function init()
  ...
  optionsTabBar:addTab(tr('GRAPHIC'), graphicsPanel, '')

  audioPanel = g_ui.loadUI('audio')
  optionsTabBar:addTab(tr('AUDIO'), audioPanel, '')

  configPanel = g_ui.loadUI('config')
  optionsTabBar:addTab(tr('CONFIG'), configPanel, '')

  chatsPanel = g_ui.loadUI('chat')
  optionsTabBar:addTab(tr('CHAT'), chatsPanel, '')
  ...
  optionsButton = modules.client_topmenu.addLeftButton('optionsButton', tr('Opcoes'), '/images/TOPBUTTONS_REWORK/ICON_CONFIG', toggle)
...
function setup()
  ...
  displayCycleState = g_settings.getNumber('displayCycleState')
  if displayCycleState < 1 or displayCycleState > 4 then
    displayCycleState = 4
  end
  applyCreatureDisplayState(displayCycleState)
...
function toggleDisplays()
  displayCycleState = displayCycleState + 1
  if displayCycleState > 4 then
    displayCycleState = 1
  end
  applyCreatureDisplayState(displayCycleState)
end

function applyCreatureDisplayState(state)
  -- State 1: hide all
  -- State 2: health bar only (no name, no xp)
  -- State 3: health bar + name (no xp)
  -- State 4: health bar + name + xp bar
  local gameMapPanel = modules.game_interface.getMapPanel()
  local showHealth = state >= 2
  local showNames = state >= 3
  local showXp = state == 4

  setOption('displayNames', showNames)
  setOption('displayHealth', showHealth)
  gameMapPanel:setDrawManaBar(showXp)
  g_settings.set('displayCycleState', state)
end
...
function setOption(key, value, force)
  ...
  elseif key == 'pokebar' then
    if modules.game_poketeam and modules.game_poketeam.getPokemonBar() then
      modules.game_poketeam.getPokemonBar():setVisible(value)
    end
    if modules.game_pokebar and modules.game_pokebar.getPokemonBar() then
  ...
  if key == 'topBar' and modules.game_topbar then
