...
function init()
	...
	mainWindow:hide()
	btn = modules.client_topmenu.addRightGameToggleButton('task_kill', tr('Task Kill'), '/images/TOPBUTTONS_REWORK/ICON_TASK', toggle)
...
function receiveOpcode(protocol, opcode, buffer)
	...
	if data_type == "parseTasks" then
		...
		for i, task in ipairs(tasks) do
			...
			widget.taskData = task -- Armazenar dados completos da task
			...
			if i == activeTask then
				...
				widget.taskButtons.onClick = function()
					-- Validar acesso antes de enviar
					if not task.access then
						displayInfoBox("Requisito não cumprido", task.descRequeriment)
						return
					end
			...
			if not task.access and not done then
				widget.sobrepor:setVisible(true)
				widget.sobrepor:raise()
				widget.sobrepor.requisitoText:setText("BLOQUEADO: " .. task.descRequeriment)
				widget.sobrepor.requisito:setImageSource("images/window_list/type/icon")
			end
	...
	elseif data_type == "killTask" then
		...
		if data.kills >= widget.killRequeriment then
			widget.sobrepor:setVisible(true)
			widget.sobrepor:raise()
			widget.sobrepor.requisitoText:setText("COMPLETA! Colete sua recompensa!")
			widget.sobrepor.requisito:setImageSource("images/window_list/done")
			widget.sobrepor.requisito:setSize('64 64')
		end
	end
